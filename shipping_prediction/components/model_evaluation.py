...
class ModelEvaluation:
    ...
    def initiate_model_evaluation(self) -> artifact_entity.ModelEvaluationArtifact:
        try:
            ...
            y_true=np.log(test_df[TARGET_COLUMN]+1)
            ...
            print(f"First 5 Predictions using previously trained model: {np.exp(y_pred[:5]) - 1}")
            ...
            current_model_r2 = r2_score(y_true=y_true, y_pred=y_pred_current)
            logging.info(f"R2 score using the current trained model: {current_model_r2}")

            # Calculate and print the first 5 predictions
            print(f"First 5 Predictions: {np.exp(y_pred_current[:5]) - 1}")
            #Exponentiate each element and subtract 1, then slice the first 5 predictions

           
            current_model_mse = mean_squared_error(y_true=y_true, y_pred=y_pred_current)
            ...
            model_eval_artifact = artifact_entity.ModelEvaluationArtifact(
                is_model_accepted=True,
                is_accuracy_improved=current_model_adj_r2 - previous_model_adj_r2
            )
            logging.info(f"Model evaluation artifact: {model_eval_artifact}")
            return model_eval_artifact
